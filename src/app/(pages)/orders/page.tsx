            ...
            <div className="container mx-auto max-w-4xl space-y-8">
                ...
                {orders.length === 0 ? (
                    ...
                    <div className="space-y-6">
                        {orders.map((order) => (
                            <Card key={order.id} className="overflow-hidden border-0 shadow-md hover:shadow-lg transition-shadow bg-white">
                                <CardHeader className="bg-gray-50/80 border-b border-gray-100 py-4">
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        <div className="space-y-1">
                                            <CardTitle className="text-lg font-medium text-gray-700">Order #{order.id}</CardTitle>
                                    ...
                                    <div className="space-y-6">
                                        ...
                                        <div className="space-y-4">
                                            {order.cartItems.map((item) => (
                                                <div key={item._id} className="flex items-center gap-4">
                                                    <div className="relative w-16 h-16 rounded-lg border border-gray-100 bg-white p-1 flex-shrink-0">
                                                        <Image
                                        ...
                                        <div className="flex flex-col md:flex-row justify-between gap-6 text-sm">
                                            <div className="space-y-1">
                                                ...
                                                <p className="text-gray-500">{order.shippingAddress.city} - {order.shippingAddress.phone}</p>
                                            </div>
                                            <div className="space-y-1 text-right">
                                                <p className="font-semibold text-gray-700 flex items-center justify-end gap-2">
                                                    <CreditCard className="w-4 h-4" /> Payment Method
                                                </p>
                                                <p className="text-gray-500 capitalize">{order.paymentMethodType}</p>
                                            </div>
                                        </div>
